...
class Arguments:
    def __init__(self):
        ...
        self.if_hybrid = 0  # {1,0} 1 refers to two types of descriptors 0 otherwise
        self.representationA = "rxnfp"
        self.representationB = "morgan_pka"  # {"morgan_fp", "one_hot", "morgan_pka","ohe_pka"}

        self.representation = "rxnfp"  # {"morgan_fp", "one_hot","Mordred","morgan_pka","ohe_pka"}
...
def main():
    ...
    save_path = os.path.join("./results/real_4/split_" + str(args.split_mode))
    ...
    y_test = orig_y_test_unnormalized
    predictionses = []
    predictions = predict(x_test, model_85_new, preprocessor_85_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_70_new, preprocessor_70_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_60_new, preprocessor_60_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_50_new, preprocessor_50_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_40_new, preprocessor_40_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_30_new, preprocessor_30_new)
    predictionses.append(predictions)
    predictions = predict(x_test, model_15_new, preprocessor_15_new)
    predictionses.append(predictions)
    
    level_labels = []
    for i in range(len(x_test)):
        level_label = len(predictionses)
        for j in range(len(predictionses)):
            if predictionses[j][i]:
                level_label = j
                break
        level_labels.append(len(predictionses)-level_label)
        
    boundarys = [85,70,60,50,40,30,15]     
    level_reals = []
    for i in range(len(y_test)):
        level_real = len(boundarys)
        for j in range(len(boundarys)):
            if y_test[i]>boundarys[j]:
                level_real = j
                break
        level_reals.append(len(boundarys)-level_real)
    
    level_olds = []
    for i in range(len(x_test)):
        level_old = len(predictionses)
        if predictionses[3][i]:
            if predictionses[1][i]:
                if predictionses[0][i]:
                    level_old=7
                else:
                    level_old=6
            elif predictionses[2][i]:
                level_old=5
            else:
                level_old=4
        else:
            if predictionses[5][i]:
                if predictionses[4][i]:
                    level_old=3
                else:
                    level_old=2
            elif predictionses[6][i]:
                level_old=1
            else:
                level_old=0
        level_olds.append(level_old)
    
    old_predictionses = []
    predictions = predict(x_test, model_85, preprocessor_85)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_70, preprocessor_70)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_60, preprocessor_60)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_50, preprocessor_50)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_40, preprocessor_40)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_30, preprocessor_30)
    old_predictionses.append(predictions)
    predictions = predict(x_test, model_15, preprocessor_15)
    old_predictionses.append(predictions)
    
    old_level_olds = []
    for i in range(len(x_test)):
        level_old = len(predictionses)
        if old_predictionses[3][i]:
            if old_predictionses[1][i]:
                if old_predictionses[0][i]:
                    level_old=7
                else:
                    level_old=6
            elif old_predictionses[2][i]:
                level_old=5
            else:
                level_old=4
        else:
            if old_predictionses[5][i]:
                if old_predictionses[4][i]:
                    level_old=3
                else:
                    level_old=2
            elif old_predictionses[6][i]:
                level_old=1
            else:
                level_old=0
        old_level_olds.append(level_old)
    
    sum1=sum([abs(level_olds[i]-level_reals[i]) for i in range(len(level_olds))])
    sum2=sum([abs(level_labels[i]-level_reals[i]) for i in range(len(level_olds))])
    sum3=sum([abs(3.5-level_reals[i]) for i in range(len(level_olds))])
    sum4=sum([abs(old_level_olds[i]-level_reals[i]) for i in range(len(level_olds))])
    
    sum11=sum([level_olds[i]==level_reals[i] for i in range(len(level_olds))])
    sum22=sum([level_labels[i]==level_reals[i] for i in range(len(level_olds))])
    sum33=sum([0==level_reals[i] for i in range(len(level_olds))])
    sum44=sum([old_level_olds[i]==level_reals[i] for i in range(len(level_olds))])
    
    upB=[15,30,40,50,60,70,85,100]
    downB=[0,15,30,40,50,60,70,85]
    
    def boundDistance(id,yyy):
        if yyy > downB[id] and yyy < upB[id]:
            return 0
        else:
            return min(abs(yyy-downB[id]),abs(yyy-upB[id]))
        
    sum111=sum([boundDistance(level_olds[i],y_test[i]) for i in range(len(level_olds))])
    sum222=sum([boundDistance(level_labels[i],y_test[i]) for i in range(len(level_olds))])
    sum333=sum([boundDistance(old_level_olds[i],y_test[i]) for i in range(len(level_olds))])
    
    with open("./record.csv","a+") as f:
        f.write(str(args.split_mode)+",")
    ...
    for i in range(len(svr_pred)):
        if svr_pred[i]<0:
            ...
            svr_pred[i]*=100
    r2_svr = 1 - np.sum((y_test - svr_pred)**2) / np.sum((y_test - np.mean(y_test))**2)
    print(r2_svr)
    with open("svr.csv",'w') as f:
        f.write("pred,real\n")
        for i in range(len(svr_pred)):

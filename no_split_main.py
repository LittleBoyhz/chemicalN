...
class Arguments:
    def __init__(self):
        ...
        self.representationA = "morgan_fp"
...
def main():
    ...
    svr_model = SVR(kernel='poly',degree=50)
    ...
    for i in range(len(x_test)):
        input_fnn = torch.tensor(x_test[i])
        input_fnn = input_fnn.to(dtype=torch.double)
        output = fnn_model(input_fnn)
    ...
    r2_fnn = r2_score(y_test,fnn_pred)
    mse_fnn = mean_squared_error(y_test,fnn_pred)
    mae_fnn = mean_absolute_error(y_test,fnn_pred)

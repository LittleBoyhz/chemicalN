...
class Margin(SamplingMethod):
    """This is an implementation of the Margin Sampling algorithm.
    ...
    def query(self, budget, *args, **kwargs):
        """Selects data points based on the margin scores, i.e., the difference between the probabilities of the first
    ...
    def _grading(self, model, preprocessor, *args, **kwargs):
        """Calculates and returns the difference between the probabilities of the first and second most probable classes.
        ...
        uncertainty = 1 - np.absolute(probabilities[:, 0] - probabilities[:, 1])  # uncertainty = 1 - margin
        return uncertainty
